...
- **Status vs closesAt**: a passed `Closes At` blocks check-ins, hides the meeting from `action=resolve` and Show QR, and makes `doGet` report `closed` (all via `isMeetingOpen`) — but the Status cell stays `open` until Close Meeting flips it, or until `closeExpiredMeetings()` (a 15-min time trigger installed via the Discord Announcements menu) flips it and posts an attendance summary to the Discord webhook. Announcing is fire-and-forget (try/catch) — Discord being down must never break a close. Close Meeting deliberately lists expired-but-open rows so they can be tidied by hand when the trigger isn't installed.
