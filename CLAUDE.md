...
- `Meetings` tab (index): Token, Meeting Name, Tab Name, Status, Opens At, Closes At, Created At, Check-in URL, Code, Reminder Sent (stamped by the trigger when the pre-meeting Discord reminder posts — only after Discord accepts, so failed posts retry on the next run)
...
- **Status vs closesAt**: a passed `Closes At` blocks check-ins, hides the meeting from `action=resolve` and Show QR, and makes `doGet` report `closed` (all via `isMeetingOpen`) — but the Status cell stays `open` until Close Meeting flips it, or until `closeExpiredMeetings()` (a 15-min time trigger installed via the Discord Announcements menu) flips it and posts an attendance summary to the Discord webhook. The same trigger also posts a reminder with the check-in link + backup code in the hour before a meeting's Opens At (keep the trigger handler named `closeExpiredMeetings` — installed triggers reference it by name). Announcing is fire-and-forget (try/catch) — Discord being down must never break a close. Close Meeting deliberately lists expired-but-open rows so they can be tidied by hand when the trigger isn't installed.
